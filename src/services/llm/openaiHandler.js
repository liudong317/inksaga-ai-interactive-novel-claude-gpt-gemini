...
async function callOpenAI(messages, provider, options = {}) {
  const maxTokens = 128000;
  const originalTokens = estimateMessagesTokenCount(messages);
  
  if (originalTokens > maxTokens) {
    console.log(`OpenAI请求token数: ${originalTokens}, 超过限制${maxTokens}, 正在截断...`);
    ...
    console.log(`OpenAI请求token数: ${originalTokens}`);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    if (provider.apiKey) {
      reqOptions.headers['Authorization'] = `Bearer ${provider.apiKey}`;
    }
      ...
      res.on('end', () => {
        try {
          ...
          if (jsonData && jsonData.choices && jsonData.choices[0] && jsonData.choices[0].message) {
            ...
            reject(new Error('OpenAI返回数据格式错误'));
          }
        } catch (error) {
          reject(new Error(`解析OpenAI响应失败: ${error.message}`));
        }
      });
    });

    req.on('error', (error) => {
      reject(new Error(`OpenAI调用失败: ${error.message}`));
    });
...
function cleanOpenAIJSON(jsonStr) {
...
module.exports = { callOpenAI, cleanOpenAIJSON };
